...
dir_restore = 'model/flownet/flownet_train/' #model/flownet_simple/train_1/'
dir_dataset = 'FlyingChairs_release/data/'
dir_test = 'test/flownet_21032021/'
...
def main(_):
    ...
    with tf.Session(config=model.tf_config) as sess:
        ...
        saver = tf.train.import_meta_graph('model/flownet/flownet_train/model-10.meta')
        saver.restore(sess,  tf.train.latest_checkpoint('model/flownet/flownet_train/'))
        for iteration in range(len(list1_test) // batch_size):
            ...
            sio.savemat((dir_test + 'flow_batch_%d' % iteration + '.mat'), {'flow':predict_out})
